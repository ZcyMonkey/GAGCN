...
class ConvTemporalGraphical(nn.Module):
    ...
    def __init__(self,
        ...
        self.T=nn.Parameter(torch.FloatTensor(self.num_experts,joints_dim , time_dim, time_dim))
    ...
    def forward(self, x):
        ...
        h = nn.Dropout(self.nn_keep_prob)(h)
        Gating_weight = nn.functional.softmax(self.gating_layer2(h), dim=1)
        batch_size = x.size()[0]
        AT = self.get_AT(self.T,Gating_weight,batch_size)
        AS = self.get_AS(self.A,Gating_weight,batch_size)
