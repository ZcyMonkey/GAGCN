...
def test():
  ...
  for action in actions:
    running_loss=0
    n=0
    dataset_test = datasets.Datasets(args.data_dir,args.input_n,args.output_n,args.skip_rate, split=0,actions=[action])
    ...
    for cnt,batch in enumerate(test_loader):
      with torch.no_grad():
        ...
        #     if(name == 'st_gcnns.3.gcn.A' ):
        #         sam= parameters.data.cpu().numpy()
        #         a = sam[1,9,:]*1000000000
        #     if(name == 'st_gcnns.3.gcn.T'):
        #         tam= parameters.data.cpu().numpy()
        #         b = tam[1,21,:]
