...
# Left / right indicator
    ...
    for i in np.arange( len(I) ):
        ...
        if not update:
            ...
            if i ==0:
                plots.append(ax.plot(x, y, z, lw=2.5,linestyle='-' ,c=lcolor if LR[i] else rcolor))
            else:
                plots.append(ax.plot(x, y, z, lw=2.5,linestyle='-', c=lcolor if LR[i] else rcolor))
    ...
    for action in actions:
        ...
        for cnt,batch in enumerate(loader): 
            ...
            for i in range(output_n):
                ...
                plt.savefig('/home/zhongchongyang/Gating-GCN/fig/v2/walking/'+x+'gt'+str(action) + ' frame:{:d}'.format(i+ 1)+'.png')
            plt.close()
            fig = plt.figure(figsize=(3,4))
            ax = Axes3D(fig)
            ax.view_init(elev=30, azim=40)
            ...
            for i in range(output_n):
                ...
                plt.savefig('/home/zhongchongyang/Gating-GCN/fig/v2/walking/'+x+'pre'+str(action) + ' frame:{:d}'.format(i+ 1)+'.png')
            plt.close()
